...
file_handler = TimedRotatingFileHandler(
    "logs/chainlit.log", when="midnight", interval=1
)
...
if not missing_vars:
    try:
        morpheus_bot = MorpheusBot(
            system_prompt=system_prompt, notebook_filename="notebook.md"
        )
...
async def clear_chat_callback(action):
    ...
    if bot:
        ...
        await cl.Message(
            content="Conversation history has been cleared. Let's start fresh!"
        ).send()
    else:
        await cl.Message(
            content="⚠️ Bot is not properly initialized. Please check your environment configuration."
        ).send()
    ...
    try:
        ...
        async with bot.agent.run_mcp_servers():
            with cl.Step(name="Processing request") as step:
                result = await bot.agent.run(
                    message.content, message_history=bot.get_history()
                )
                        ...
                        if hasattr(part, "has_content") and part.has_content():
                            if hasattr(part, "tool_name"):  # For ToolCallPart
                                tool_call_content = {
                                    ...
                                    "result": (
                                        part.content if hasattr(part, "content") else ""
                                    ),
                                }
